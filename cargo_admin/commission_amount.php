...
case 'word':
	...
	header('Content-Disposition: attachment; filename=CommissionAmount.doc');
	export_Word_Excel();
	exit();
	break;
case 'excel':
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment; filename=CommissionAmount.xls');
	export_Word_Excel();
	exit();
	break;
case 'xml':	
	header('Content-Type: text/xml');
	header('Content-Disposition: attachment; filename=CommissionAmount.xml');
	export_xml($export);
	exit();
	break;
case 'csv':	
	header('Content-Type: application/csv');
	header('Content-Disposition: attachment; filename=CommissionAmount.csv');
	export_csv($export);
	exit();
	break;
}
...
<form action="commission_amount.php" method="post" style="background-image:url(hfddfg.jpg)">
<table width="93%" height="152"  border="0" align="center" cellpadding="4" cellspacing="1">
<tr>
	...
	<td class="td1"><strong>Lorry Owner
	    <select name="truck_owner_id" id="truck_owner_id">
...
<td><a href="commission_amount.php?a=reset">Reset Search </a></td>
<td>&nbsp;</td>
</tr>
</table>
</form>
...
<tr>
	<td class="hr"><font color="#FFFFFF">Si.No</font></td>
	<td class="hr"><font color="#FFFFFF">C. M. No</font></td>
	...
	<td class="hr"><font color="#FFFFFF">Commission Amount</font></td>
</tr>
...
<?php
  for ($i = $startrec; $i < $reccount; $i++)
  {
    ...
    $style = "dr";
	 ...
	 //Query to find the commission amount of the perticular memo
	 $sql_amt = "select amount from memo_amount where perticulars = 'Commission Charges' and commission_id = ".$row['commission_id'];
...
<td class="<?php echo $style ?>"><?php echo htmlspecialchars(indian_cur($row["advance"])) ?></td>
</tr>
<?php
  }
...
<tr>
	<td colspan="7"></td>
	<td class="hr">Grand Total : <?php echo indian_cur(calc_commission_amount()); ?></td>
</tr>
</table>
<br>
<table width="100%" align="center" >
		<tr height="20" valign="middle"><td align="right"><a href="commission_amount.php?a=word&order=<?php echo $order ?>&type=<?php echo $ordtype ?>&f=<?php echo $f ?>" title="Microsoft WORD Format" style="text-decoration:none;">Export to Word</a><font style="color:#000000; font-weight:bold;"> &brvbar; </font><a href="commission_amount.php?a=excel&order=<?php echo $order ?>&type=<?php echo $ordtype ?>&f=<?php echo $f ?>" title="Microsoft EXCEL Format" style="text-decoration:none;">Export to Excel</a>&nbsp;&nbsp;&nbsp;</td></tr>
</table>
...
<tr>
<td>&nbsp;</td>
<td><a onclick="return confirm('Are you sure you want to delete this item?');" href="commission_amount.php?a=del&recid=<?php echo $recid ?>&order=<?php echo $order ?>&type=<?php echo $ordtype ?>&f=<?php echo $f ?>">Delete Record</a></td>
</tr>
</table>
<?php
  mysql_free_result($res);
...
?>
<br>
<form enctype="multipart/form-data" action="commission_amount.php" method="post" onsubmit="if(isFormValid()) ; else { alert('Something is wrong');return false; }">
...
?>
<br>
<form action="commission_amount.php" method="post">
...
?>
<br />
<table width="70%" border="0" cellpadding="5" cellspacing="1" align="center" id="memo_amount">
	<tr>
		<td colspan="6" align="left" class="hr">Amount Details</td>
	</tr>
  <tr height="25">
    <td class="hr"><font color="#FFFFFF">Date</font></td>
	<td class="hr"><font color="#FFFFFF">Amount</font></td>
	<td class="hr"><font color="#FFFFFF">Perticulars</font></td>
	<td class="hr"><font color="#FFFFFF">Acc/No</font></td>
	<td class="hr"><font color="#FFFFFF">Challan No</font></td>
	<td class="hr"><font color="#FFFFFF"></font></td>
  </tr>
  <?php
  $i=0;
  while($row1 = mysql_fetch_array($res1))
  {
	 ...
	 $i++;
  ?>
  	<tr bgcolor="<?php echo $style1;?>">
		<td class="td1"><?php echo FormatDate($row1['date'])?></td>
		<td class="td1"><?php echo $row1['amount']?></td>
		<td class="td1"><?php echo $row1['perticulars']?></td>
		<td class="td1"><?php 
		if($row1['acc_no'] != "")
		{
			//Query to fetch transporter company name
			$sql_accno = "select acc_no from truck_owner_acc where acc_id = ".$row1['acc_no'];
			$res_accno = mysql_query($sql_accno);
			$row_accno = mysql_fetch_array($res_accno);
			
			echo $row_accno['acc_no'];
		} else
		{
			echo "-NA-";
		}
		...
		if($row1['challan_no'] != "")
		{	
			echo $row1['challan_no'];
		} else
		{
			echo "-NA-";
		}
		?></td>
		<td><a href="#" onClick="openWindow('edit_memo_amount.php?id=<?php echo $row1['memo_amount_id']?>&c_id=<?php echo $row['commission_id']?>');">Edit</a>&nbsp;&nbsp;|&nbsp;&nbsp;
		<?php if($row1['perticulars'] == "Commission Charges") { echo "<a href=\"#\">Delete</a>"; } else { ?>
		<a href="#" onClick="deleteRec(this.parentNode.parentNode.rowIndex,'<?php echo $row1['memo_amount_id']?>');">Delete</a><?php }?></td>
	</tr>
  <?php	
  $i++;
  }
  ?>
	...
	<tr>
	<?php
		$sql_amt = "select sum(amount) as amt from memo_amount where commission_id = ".$row['commission_id'];
		$res_amt = mysql_query($sql_amt);
		$row_amt = mysql_fetch_array($res_amt);
	?>
		<td colspan="2" align="left" class="hr">Balance Amount : <?php echo ($row['fright_charges'] - $row_amt['amt']);?></td>
			</tr>
</table>
...
<table   border="0" cellspacing="1" cellpadding="4" align="center" width="100%">
<tr>
<td><a href="commission_amount.php?order=<?php echo $order ?>&type=<?php echo $ordtype ?>&f=<?php echo $f ?>">Index Page</a></td>
<?php if ($recid > 0) { ?>
<td><a href="commission_amount.php?a=<?php echo $a ?>&recid=<?php echo $recid - 1 ?>&order=<?php echo $order ?>&type=<?php echo $ordtype ?>&f=<?php echo $f ?>">Prior Record</a></td>
<?php } if ($recid < $count - 1) { ?>
<td><a href="commission_amount.php?a=<?php echo $a ?>&recid=<?php echo $recid + 1 ?>&order=<?php echo $order ?>&type=<?php echo $ordtype ?>&f=<?php echo $f ?>">Next Record</a></td>
<?php } ?>
</tr>
</table>
<hr size="1" noshade width="100%" align="center">
...
<?php
//--------------------------------------------------------------------------------------------------------------------------------------------
//Function to calculate the entire commission amount depending upon the query
function calc_commission_amount()
{
	...
	{
		//Query to calculating only commission amount based on the filter applied
		$sql_amt = "select advance from commission_memo where commission_id = ".$row['commission_id'];
		$res_amt = mysql_query($sql_amt);
		$row_amt = mysql_fetch_array($res_amt);
		$commission_amount += $row_amt['advance'];
	}
...
<td>&nbsp;
<?php if ($page > 1) { ?>
<a href="commission_amount.php?page=<?php echo $page - 1 ?>&order=<?php echo $order ?>&type=<?php echo $ordtype ?>&f=<?php echo $f ?>">&lt;&lt;&nbsp;Prev</a>&nbsp;
<?php } ?>
<?php
  global $pagerange;
...
<b><?php echo $j ?></b>
<?php } else { ?>
<a href="commission_amount.php?page=<?php echo $j ?>&order=<?php echo $order ?>&type=<?php echo $ordtype ?>&f=<?php echo $f ?>"><?php echo $j ?></a>
<?php } } } else { ?>
<a href="commission_amount.php?page=<?php echo $startpage ?>&order=<?php echo $order ?>&type=<?php echo $ordtype ?>&f=<?php echo $f ?>"><?php echo $startpage ."..." .$count ?></a>
<?php } } } ?>
<?php if ($page < $pagecount) { ?>
&nbsp;<a href="commission_amount.php?page=<?php echo $page + 1 ?>&order=<?php echo $order ?>&type=<?php echo $ordtype ?>&f=<?php echo $f ?>">Next&nbsp;&gt;&gt;</a>&nbsp;</td>
<?php } ?>
</tr>
</table>
<?php 
...
<tr bgcolor="#487698"> 
	...
	<td class="hr"><font color="#FFFFFF"><strong>Commission Amount</strong></font></td>
</tr>
...
<td class="<?php echo $style ?>"><?php echo htmlspecialchars(FormatDate($row["date"])) ?></td>
<td class="<?php echo $style ?>"><?php echo htmlspecialchars(indian_cur($row["fright_charges"])) ?></td>
<td class="<?php echo $style ?>"><?php echo htmlspecialchars(indian_cur($row_amt['amount'])) ?></td>
</tr>
<?php
  }
  mysql_free_result($res);
...
<tr>
	<td colspan="6"></td>
	<td bgcolor="#487698"><strong><font color="#FFFFFF">Grand Total : <?php echo indian_cur(calc_commission_amount()); ?></font></strong></td>
</tr>
</table>
